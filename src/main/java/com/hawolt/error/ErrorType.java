...
public enum ErrorType {
    ...
    ErrorType(int id, String description) {
    ...
    private static ErrorType findById(int id) {
        for (ErrorType errorType : ERROR_TYPES) {
            if (errorType.getId() == id) return errorType;
        }
        return ErrorType.ERROR_UNKNOWN;
    }

    public static ErrorType find(JSONObject response) {
        if (!response.has("errorId")) return ErrorType.NO_ERROR;
